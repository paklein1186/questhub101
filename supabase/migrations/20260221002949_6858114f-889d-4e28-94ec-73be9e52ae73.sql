-- Fix log_follow to handle TOPIC target_type
...
BEGIN
  ...
  ELSIF NEW.target_type = 'TOPIC' THEN
    SELECT name INTO t_name FROM public.topics WHERE id = NEW.target_id::uuid;
  ELSIF NEW.target_type = 'POD' THEN
    SELECT name INTO t_name FROM public.pods WHERE id = NEW.target_id::uuid;
  ELSIF NEW.target_type = 'EVENT' THEN
    SELECT title INTO t_name FROM public.events WHERE id = NEW.target_id::uuid;
  END IF;
...
-- Backfill missing target_name for topic follows
UPDATE activity_log al
SET target_name = t.name
FROM topics t
WHERE al.target_type = 'topic'
  AND al.action_type = 'followed'
  AND al.target_name IS NULL
  AND t.id = al.target_id::uuid;