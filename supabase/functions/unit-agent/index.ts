...
function buildSystemPrompt(entityType: string, entityName: string, contextSummary: string) {
...
async function gatherContext(supabase: any, entityType: string, entityId: string): Promise<{ name: string; summary: string }> {
  ...
  try {
    if (entityType === "GUILD") {
      ...
      if (members?.length) {
        parts.push(`Members (${members.length}): ${members.map((m: any) => `${m.profiles?.name || "?"} (${m.role})`).join(", ")}`);
      }
...
serve(async (req) => {
  ...
  try {
    const { entityType, entityId, message, conversationHistory } = await req.json();
    ...
    const systemPrompt = buildSystemPrompt(entityType, entityName, contextSummary);
    ...
    if (conversationHistory?.length) {
      ...
      for (const msg of recent) {
        messages.push({
          ...
          content: msg.sender_type === "USER" ? `[${msg.sender_name || "User"}]: ${msg.message_text}` : msg.message_text,
        });
      }
    }
    ...
    // Parse structured suggestions from the reply
    ...
    // Store agent message in the thread
    ...
    if (thread) {
      await supabase.from("unit_chat_messages").insert({
        ...
        metadata_json: suggestions.length > 0 ? { suggestions } : {},
      });
    }
