      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground mb-4">
          ...
          {svc.priceAmount != null && (
            <Badge className="bg-primary/10 text-primary border-0"><Euro className="h-3 w-3 mr-0.5" />{svc.priceAmount === 0 ? "Free" : `${svc.priceAmount} ${svc.priceCurrency}`}</Badge>
          )}
              ...
              <div className="space-y-4 mt-2">
                ...
                {providerUserId ? (
                  Object.keys(slotsByDate).length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-4">No available slots this week. Try another week.</p>
                  ) : (
                      ...
                      {Object.entries(slotsByDate).map(([date, daySlots]) => (
                        <div key={date}>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {daySlots.map(slot => (
                              <Button key={slot.startDateTime} size="sm" variant={selectedSlot?.startDateTime === slot.startDateTime ? "default" : "outline"} className="text-xs" onClick={() => setSelectedSlot(slot)}>{slot.label}</Button>
                            ))}
                          </div>
                        </div>
                      ))}
                  ...
                  <p className="text-sm text-muted-foreground">This is a guild-provided service. Availability is managed by the guild admins.</p>
                )}
                <div>
                ...
                <Button onClick={createBooking} className="w-full" disabled={providerUserId ? !selectedSlot : false}>
                  <Send className="h-4 w-4 mr-1" />{isFree ? "Confirm (Free)" : `Book & Pay (€${svc.priceAmount})`}
                </Button>
