            ...
            {companyQuests.map(quest => (
              <Link key={quest.id} to={`/quests/${quest.id}`} className="block rounded-xl border border-border bg-card p-4 hover:border-primary/30 transition-all">
                ...
                <div className="flex gap-1.5 mt-2">
                  <Badge className="bg-accent text-accent-foreground border-0 text-[10px]"><Building2 className="h-2.5 w-2.5 mr-0.5" />Client quest</Badge>
              ...
              return (
                <div key={booking.id} className="rounded-lg border border-border bg-card p-4">
                  <div className="flex items-center justify-between">
                    <Link to={`/services/${booking.serviceId}`} className="font-medium hover:text-primary transition-colors">{svc?.title}</Link>
