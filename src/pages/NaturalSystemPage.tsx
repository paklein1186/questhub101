...
import {
  ...
  TrendingUp, AlertTriangle, BarChart3, ArrowLeft, Globe, Network
} from "lucide-react";
...
import { NsEcosystemTab } from "@/components/natural-system/NsEcosystemTab";
...
/** Resolve territory names for linked territory IDs */
function useTerritoryNames(territoryIds: string[]) {
  return useQuery<Record<string, string>>({
    queryKey: ["territory-names", territoryIds],
...
export default function NaturalSystemPage() {
  ...
  const territoryLinks = useMemo(() => (links ?? []).filter(l => l.linked_type === "territory"), [links]);
  ...
  const territoryLinkIds = useMemo(() => territoryLinks.map(l => l.linked_id), [territoryLinks]);
  const { data: profiles } = useProfilesByIds(userIds);
  const { data: guilds } = useGuildsByIds(guildIds);
  const { data: territoryNames } = useTerritoryNames(territoryLinkIds);
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="overview" className="space-y-10 mt-6">
          ...
          <section className="space-y-5">
            ...
            {/* Governance relationships — with resolved territory names */}
            <Card>
              <CardContent className="p-5 space-y-3">
                <h3 className="font-display font-semibold text-sm flex items-center gap-2">
                  ...
                  {ns.territory_id && territoryName && (
                    <Link to={`/territories/${ns.territory_id}`}>
                      <Badge variant="outline" className="gap-1 cursor-pointer hover:bg-muted/50">
                        <MapPin className="h-3 w-3" /> {territoryName}
                      </Badge>
                    </Link>
                  )}
                  {territoryLinks.map(l => {
                    // Skip if it's the same as the primary territory
                    if (l.linked_id === ns.territory_id) return null;
                    const tName = territoryNames?.[l.linked_id];
                    return (
                      <Link key={l.id} to={`/territories/${l.linked_id}`}>
                        <Badge variant="outline" className="gap-1 cursor-pointer hover:bg-muted/50">
                          <MapPin className="h-3 w-3" /> {tName || "Territory"}
                        </Badge>
                      </Link>
                    );
                  })}
        ...
        <TabsContent value="ecosystem" className="mt-6">
          <NsEcosystemTab naturalSystemId={id!} />
        </TabsContent>
      </Tabs>
