             ...
             {(allTopics ?? []).map(t => {
               ...
               const label = getTopicLabel(t);
                ...
                return (
                  <div
                    key={t.id}
                    className={`group relative px-3 py-2 rounded-lg border transition-all duration-150 text-sm font-medium flex items-center gap-2 justify-center hover:shadow-sm ${
                      isSelected
                  ...
                  >
                    ...
                    <Link
                      to={`/topics/${t.slug}`}
                      className={`shrink-0 opacity-0 group-hover:opacity-100 transition-opacity ${isSelected ? "text-primary-foreground/70 hover:text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
