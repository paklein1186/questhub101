...
import { TopicFeatureTargetType, CommentTargetType } from "@/types/enums";
...
export default function TopicHouse() {
  ...
  const addFeature = (targetType: TopicFeatureTargetType, targetId: string) => {
    if (!targetId) return;
    const tf: TopicFeature = { id: `tf-${Date.now()}`, topicId: topic.id, targetType, targetId, addedByUserId: currentUser.id, createdAt: new Date().toISOString() };
          ...
          <div className="flex flex-wrap gap-3">
            {stewards.map(s => (
              <Link key={s.id} to={`/users/${s.userId}`} className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 hover:border-primary/30 transition-all">
                <Avatar className="h-7 w-7"><AvatarImage src={s.user?.avatarUrl} /><AvatarFallback>{s.user?.name?.[0]}</AvatarFallback></Avatar>
                ...
                {featuredQuests.map(quest => {
                  ...
                  return (
                    <div key={quest.id} className="flex items-start gap-3 rounded-xl border border-border bg-card p-4">
                      ...
                      {isSteward && feature && (
                        <Button size="sm" variant="ghost" className="h-7 w-7 p-0 shrink-0" onClick={() => removeFeature(feature.id)}><X className="h-3.5 w-3.5 text-destructive" /></Button>
                      )}
                    </div>
                  );
                })}
                ...
                <div className="flex items-end gap-3">
                  <div className="flex-1">
                    ...
                    <Select value={featureQuestId} onValueChange={setFeatureQuestId}>
                      ...
                      <SelectContent>{unfeaturedQuests.map(q => <SelectItem key={q.id} value={q.id}>{q.title}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                  <Button size="sm" onClick={() => addFeature(TopicFeatureTargetType.QUEST, featureQuestId)} disabled={!featureQuestId}><Plus className="h-4 w-4 mr-1" /> Feature</Button>
                </div>
              )}
                ...
                <div className="flex items-end gap-3">
                  <div className="flex-1">
                    ...
                    <Select value={featureGuildId} onValueChange={setFeatureGuildId}>
                      ...
                      <SelectContent>{unfeaturedGuilds.map(g => <SelectItem key={g.id} value={g.id}>{g.name}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                  <Button size="sm" onClick={() => addFeature(TopicFeatureTargetType.GUILD, featureGuildId)} disabled={!featureGuildId}><Plus className="h-4 w-4 mr-1" /> Feature</Button>
                </div>
              )}
            </div>
          )}
