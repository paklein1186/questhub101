    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          {refCode && (
            <Badge variant="secondary" className="mt-2 gap-1"><Gift className="h-3 w-3" /> {t("auth.referredBonus")}</Badge>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-8 space-y-5">
          <Button
            ...
            onClick={async () => {
              ...
              if (error) {
                toast({ title: t("auth.googleSignUpFailed"), description: String(error), variant: "destructive" });
              }
            }}
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              ...
              <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder={t("auth.yourFullName")} required maxLength={80} autoComplete="name" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t("auth.minChars", { n: 6 })} required minLength={6} autoComplete="new-password" />
            </div>
            <div className="space-y-2">
        ...
        <p className="text-center text-sm text-muted-foreground mt-6">
          ...
          <Link to={`/login${redirectTo ? `?redirect=${encodeURIComponent(redirectTo)}` : ""}`} className="text-primary font-medium hover:underline">{t("nav.login")}</Link>
        </p>
      </motion.div>
    </div>
  );
