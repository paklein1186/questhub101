...
import { Plus, Briefcase, Building2, User, MapPin, FileText, Upload, Trash2 } from "lucide-react";
...
export function AddJobDialog({ open, onOpenChange }: Props) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    setUploading(true);
    const folder = selectedCompanyId || currentUser.id || "unknown";
    const path = `jobs/${folder}/${Date.now()}_${file.name}`;
